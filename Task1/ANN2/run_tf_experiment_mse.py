...
def run_experiment(name, use_tf, tf_decay_factor=0.0):
    ...
    RESULTS_DIR = os.path.join(BASE_DIR, 'results', 'TF_Experiment_MSE')
    ...
    print(f"\n{'='*50}\nStarting TF Experiment (MSE): {name}\nDecay Factor: {tf_decay_factor}\n{'='*50}")
    ...
    base_filename = f"TFExpMSE_{name}_{HIDDEN_DIM}units_{timestamp}"
    ...
    train_losses, val_losses = train_model(
        ...
        loss_type="MSE",  # Locking back to MSE
...
def main():
    ...
    plt.title('Teacher Forcing Decay Rates (MSE Loss Validation)')
    plt.xlabel('Epoch')
    ...
    plot_path = os.path.join(RESULTS_DIR, 'TF_Experiment_MSE_Comparison.png')
    plt.savefig(plot_path)
    print(f"\nSuccessfully finished TF (MSE) experiments!")
    ...
    compare_models("TF_Experiment_MSE")
